...
[Ссылка на датасет на Roboflow](https://app.roboflow.com/nsu-sxhmy/project_x-ar6bm/models)
...
Также на этом шаге я добавил аугментации, помимо стандартных в ultrlitycs yolo чтобы привести синтетику к более реальным данным:
```
Blur(p=0.1, blur_limit=(3, 7)),
MedianBlur(p=0.05, blur_limit=(3, 7)),
ToGray(p=0.05, method='weighted_average', num_output_channels=3),
CLAHE(p=0.05, clip_limit=(1.0, 4.0), tile_grid_size=(8, 8)),
RandomBrightnessContrast(p=0.1, brightness_by_max=True, brightness_limit=(-0.2, 0.2),
contrast_limit=(-0.2, 0.2), ensure_safe_range=False),
RandomGamma(p=0.1, gamma_limit=(80.0, 120.0)),
ImageCompression(p=0.1, compression_type='jpeg', quality_range=(25, 100)),
GaussNoise(p=0.1, mean_range=(0.0, 0.0), noise_scale_factor=1.0, per_channel=True, std_range=(0.025, 0.5))
```
...
1. В первую очередь необходимо сильно увеличить датасет (сейчас в датасете организаторов только скрншоты с сайта и с некоторой рекламы), нужно:
    ...
    4. Также синтетику можно сделать проще, поскольку лого плоское, можно набрать случайных изображений, получить через depth estimation CNN их карты глубины, по ним восстановить перспективы и тд, затем логотип по этой перспективы наложить, а так нагенерировать сколько угодно данных, таким образом можно набрать условно 25 разных лого, и из них сделать тысячи картинок на разном фоне (при этом логично обрезать лого нужно не по боксу (отрезать уголки щита, png).
